...
const analyzeFileSchema = z.object({
  ...
  templateType: z.enum(['grounds', 'followup_grounds']),
  version: z.string().default('v1')
});
 ...
 */
router.get('/validate', requireAuth, adminLimiter, async (req, res) => {
  try {
 ...
 */
router.get('/preview', requireAuth, adminLimiter, async (req, res) => {
  try {
    const { googleDocId } = validateTemplateSchema.parse(req.query);
 ...
 */
router.post('/analyze', requireAuth, adminLimiter, async (req, res) => {
  try {
...
/**
 * Analyze an uploaded template file (DOCX/PDF) and extract concerns
 * POST /api/templates/analyze-file
 */
router.post('/analyze-file', requireAuth, adminLimiter, async (req, res) => {
  try {
    const { projectId, templateType, version } = analyzeFileSchema.parse(req.body);
    ...
    const { data: templateFile, error: fileError } = await supabase
      .from('template_files')
      .select('id, active_version_id, template_versions!template_versions_template_file_id_fkey(id, storage_path, mimetype)')
    ...
    if (fileError || !templateFile) {
      return res.status(404).json({
        success: false,
        error: `No ${templateType} template found for this project. Please upload a template first.`
      });
    }

    // Get the active version
    const activeVersion = templateFile.template_versions?.find(
      (v: any) => v.id === templateFile.active_version_id
    );

    if (!activeVersion) {
      return res.status(404).json({
        success: false,
        error: 'No active template version found'
      });
    }

    // Perform analysis on the uploaded file
    const analysisResult = await getDocumentAnalysis().analyzeUploadedTemplate(
      activeVersion.storage_path,
      activeVersion.mimetype
    );

    // Save concerns to database
    let savedConcerns = null;
    try {
      savedConcerns = await getDocumentAnalysis().generateSurveyFromAnalysis(
        analysisResult,
    ...
    console.error('[analyze-file] Analysis failed:', error);
    res.status(400).json({
      success: false,
      error: error.message || 'Uploaded template analysis failed'
    });
  }
});
 ...
 */
router.post('/generate-survey', requireAuth, adminLimiter, async (req, res) => {
  try {
 ...
 */
router.post('/upload', requireAuth, adminLimiter, upload.single('file'), async (req, res) => {
  try {
    console.log('[upload] Received upload request:', { body: req.body, hasFile: !!req.file });
    ...
    // Check if version label already exists, if so append timestamp
    const { data: existingVersions } = await supabase
      .from('template_versions')
      .select('version_label')
      .eq('template_file_id', templateFile.id)
      .eq('version_label', body.version);

    let versionLabel = body.version;
    if (existingVersions && existingVersions.length > 0) {
      versionLabel = `${body.version}-${Date.now()}`;
      console.log(`[upload] Version ${body.version} exists, using ${versionLabel}`);
    }
    ...
    console.error('[upload] Template upload failed:', error);
    if (error.issues) {
 ...
 */
router.get('/concerns', requireAuth, adminLimiter, async (req, res) => {
  try {
 ...
 */
router.put('/concerns', requireAuth, adminLimiter, async (req, res) => {
  try {
 ...
 */
router.delete('/concerns/:version/:key', requireAuth, adminLimiter, async (req, res) => {
  try {
...
router.get('/files/:projectId', requireAuth, adminLimiter, async (req, res) => {
  try {
    const { projectId } = req.params;
    console.log('[files] Listing template files for project:', projectId);
    const supabase = getSupabase();
    ...
    const { data: files, error } = await supabase
      .from('template_files')
      .select('id, project_id, template_type, active_version_id, updated_at, template_versions!template_versions_template_file_id_fkey(id, version_label, storage_path, mimetype, original_filename, merge_fields, created_at)')
      .eq('project_id', projectId)
      .order('updated_at', { ascending: false });

    if (error) {
      console.error('[files] Database error:', error);
      throw new Error(error.message);
    }
    ...
    console.log('[files] Found', transformed.length, 'template files');
    res.json({ success: true, data: transformed });
  } catch (error: any) {
    console.error('[files] Failed to list template files:', error);
    res.status(400).json({ success: false, error: error.message || 'Failed to list template files' });
  }
});

router.post('/files/:fileId/activate', requireAuth, adminLimiter, async (req, res) => {
  try {
...
router.delete('/files/:fileId/version/:versionId', requireAuth, adminLimiter, async (req, res) => {
  try {
 ...
 */
router.put('/email-body/:projectId', requireAuth, adminLimiter, async (req, res) => {
  try {
    const { projectId } = req.params;
 ...
 */
router.get('/defaults/gold-coast', requireAuth, adminLimiter, async (req, res) => {
  try {
